...
const MindCaseApp = () => {
  ...
  const handleLanguageChange = (lang) => {
    ...
    // Persist settings via saveUserProgress if user is logged in
    if (user?.id) {
       const progress = ProgressService.getUserProgress(user.id);
       progress.settings = { ...progress.settings, selectedLanguage: lang };
  ...
  // Check if we should show the rate app prompt after activity completion
  const checkAndShowRatePrompt = () => {
    if (!user?.id) return;
    try {
      if (window.RateAppService && window.RateAppService.shouldShowPrompt(user.id)) {
      ...
      console.warn('[RateApp] Check failed:', e);
    }
  };
